...
internal static class YamlRuleExtensions
{
    ...
    public static T BuildRuleExpression<T>(this IList<YamlRule> yamlRules, RuleMetadata ruleMetadata, Func<BuildableExpression, T> build, Aliases aliases, Details details, ChannelAbbrevations channelAbbreviations, ProviderAbbrevations providerAbbreviations, ISet<string> domainControllers, out ISet<ushort> eventIds, out ISet<string> properties)
    {
        ...
        var channelEventIds = EnumerateChannelEventIds(yamlRules, channelAbbreviations);
        var providerEventIds = EnumerateProviderEventIds(yamlRules, providerAbbreviations);
        eventIds = channelEventIds.Select(item => item.EventId).Concat(providerEventIds.Select(item => item.EventId)).Where(value => !string.IsNullOrWhiteSpace(value)).Select(ushort.Parse).ToHashSet();
    ...
    public static RuleMetadata ToMetadata(this IList<YamlRule> yamlRules)
    {
        ...
        if (correlationRule is not null)
        {
            if (string.IsNullOrEmpty(correlationRule.Description))
            {
                ...
                return new RuleMetadata(correlationRule.Id, otherRule.Title, otherRule.Date, otherRule.Modified, otherRule.Author, otherRule.Details, otherRule.Description, otherRule.Level, otherRule.Status, otherRule.GetLink(), otherRule.Tags, otherRule.References, otherRule.FalsePositives, correlationOrAggregationTimeSpan: correlationRule.TryGetTimeframe(out var correlationTimeSpan) ? correlationTimeSpan : null);
            }
            else
            {
                return new RuleMetadata(correlationRule.Id, correlationRule.Title, correlationRule.Date, correlationRule.Modified, correlationRule.Author, correlationRule.Details, correlationRule.Description, correlationRule.Level, correlationRule.Status, correlationRule.GetLink(), correlationRule.Tags, correlationRule.References, correlationRule.FalsePositives, correlationOrAggregationTimeSpan: correlationRule.TryGetTimeframe(out var correlationTimeSpan) ? correlationTimeSpan : null);
            }
        }

        var rule = yamlRules.Single();
        return new RuleMetadata(rule.Id, rule.Title, rule.Date, rule.Modified, rule.Author, rule.Details, rule.Description, rule.Level, rule.Status, rule.GetLink(), rule.Tags, rule.References, rule.FalsePositives, correlationOrAggregationTimeSpan: rule.TryGetTimeframe(out var aggregationTimeSpan) ? aggregationTimeSpan : null);
    }

    extension(YamlRule yamlRule)
    {
        public RuleMetadata ToMetadata()
        {
            if (yamlRule.IsCorrelation())
            {
                return new RuleMetadata(yamlRule.Id, yamlRule.Title, yamlRule.Date, yamlRule.Modified, yamlRule.Author, yamlRule.Details, yamlRule.Description, yamlRule.Level, yamlRule.Status, yamlRule.GetLink(), yamlRule.Tags, yamlRule.References, yamlRule.FalsePositives, correlationOrAggregationTimeSpan: yamlRule.TryGetTimeframe(out var correlationTimeSpan) ? correlationTimeSpan : null);
            }

            return new RuleMetadata(yamlRule.Id, yamlRule.Title, yamlRule.Date, yamlRule.Modified, yamlRule.Author, yamlRule.Details, yamlRule.Description, yamlRule.Level, yamlRule.Status, yamlRule.GetLink(), yamlRule.Tags, yamlRule.References, yamlRule.FalsePositives, correlationOrAggregationTimeSpan: yamlRule.TryGetTimeframe(out var aggregationTimeSpan) ? aggregationTimeSpan : null);
        }

        private bool TryGetTimeframe(out TimeSpan timeframe)
        {
            timeframe = Constants.DefaultTimeFrame;
        ...
        public bool TryGetConditionNode([MaybeNullWhen(false)] out string conditionNode)
        {
            conditionNode = null;
        ...
        {
            if (yamlRule.TryGetFromCorrelation<IDictionary<string, object>>(Constants.Condition, out var condition))
            {
        ...
        public string[] GetCorrelationDimensions()
        {
            if (yamlRule.TryGetFromCorrelation<string>(Constants.GroupBy, out var groupBy))
            {
                return [groupBy];
            }
        
            if (yamlRule.TryGetFromCorrelation<IEnumerable<string>>(Constants.GroupBy, out var groupByEnumerable))
            {
        ...
        public string[] GetCorrelationRules()
        {
            if (yamlRule.TryGetFromCorrelation<string>(Constants.Rules, out var rules))
            {
                return [rules];
            }
        
            if (yamlRule.TryGetFromCorrelation<IEnumerable<string>>(Constants.Rules, out var rulesEnumerable))
            {
        ...
        public string GetCorrelationOperator(out string value)
        {
            value = string.Empty;
            if (yamlRule.TryGetFromCorrelation<IDictionary<string, object>>(Constants.Condition, out var condition))
            {
        ...
        private bool TryGetFromDetection<T>(string item, [MaybeNullWhen(false)] out T result) where T : notnull
        {
            result = default;
        ...
        private bool TryGetFromCorrelation<T>(string item, [MaybeNullWhen(false)] out T result) where T : notnull
        {
            result = default;
        ...
        public bool IsCorrelation() => yamlRule.Correlations.Count != 0;
    }
}