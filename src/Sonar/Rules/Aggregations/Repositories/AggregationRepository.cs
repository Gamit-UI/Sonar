...
using Sonar.Rules.Aggregations.Contexts;
using Sonar.Rules.Aggregations.Extensions;
using Sonar.Rules.Aggregations.Helpers;
using Sonar.Rules.Aggregations.Interfaces;
using Sonar.Rules.Serialization;

namespace Sonar.Rules.Aggregations.Repositories;

internal sealed class AggregationRepository(ILogger<AggregationRepository> logger, IHostApplicationLifetime hostApplicationLifetime) : IAggregationRepository
{
    ...
    private readonly ConcurrentDictionary<string, Lazy<AggregationContext>> contexts = new(StringComparer.OrdinalIgnoreCase);

    private AggregationContext GetContext(string ruleId)
    {
        return contexts.GetOrAdd(ruleId, valueFactory: id =>
        {
            return new Lazy<AggregationContext>(() =>
            {
                var context = new AggregationContext(logger, hostApplicationLifetime, id);
    ...
    public async Task InsertAsync(IDictionary<AggregationRule, IEnumerable<WinEvent>> aggregations, Action<AggregationRule, long, ISet<string>> onInsert, IPropertyStore rulePropertiesProvider, CancellationToken cancellationToken)
    {
