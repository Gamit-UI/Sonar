...
using Sonar.Rules.Builders.Executors.Helpers;
using Sonar.Rules.Extensions;

namespace Sonar.Rules.Builders.Executors;
...
internal static partial class Executor
{
    ...
    private static bool IsRegex(ReadOnlySpan<char> modifier, string expectedValue, string? value, Func<string, bool> canProcessRegex, Action<string> onRegexFailure, out bool result)
    {
        ...
        try
        {
            ...
            {
                if (value == null) return true;
                result = Regex.IsMatch(input: value, pattern: expectedValue, RegexOptions.None, matchTimeout: TimeSpan.FromSeconds(1));
                return true;
            }
            ...
            {
                if (value == null) return true;
                result = Regex.IsMatch(input: value, pattern: expectedValue, RegexOptions.IgnoreCase, matchTimeout: TimeSpan.FromSeconds(1));
                return true;
            }
            ...
            {
                if (value == null) return true;
                result = Regex.IsMatch(input: value, pattern: expectedValue, RegexOptions.Multiline, matchTimeout: TimeSpan.FromSeconds(1));
                return true;
            }
            ...
            {
                if (value == null) return true;
                result = Regex.IsMatch(input: value, pattern: expectedValue, RegexOptions.Singleline, matchTimeout: TimeSpan.FromSeconds(1));
                return true;
            }
        }
        catch (Exception)
