...
using Sonar.Rules.Builders.Executors;
...
internal static class ExpressionBuilder
{
    ...
    {
        if (Aliases.Instance.Value.Items.TryGetValue(key, out var alias))
        {
            key = alias;
        }
        
        return Executor.Match(key, values.Select(EscapeBackSlashes).ToList(), GetValue(key, parentNodeName, winEvent), aliasFactory: name => Aliases.Instance.Value.Items.TryGetValue(name, out var found) ? found : null, fieldName => GetValue(fieldName, parentNodeName, winEvent), winEvent.EnumerateStringValues, domainControllers, canProcessRegex, onRegexFailure);
    }
    ...
    private static bool Match(WinEvent winEvent, string key, string value, string? parentNodeName, ISet<string> domainControllers, Func<string, bool> canProcessRegex, Action<string> onRegexFailure)
    {
        ...
        return Executor.Match(key, EscapeBackSlashes(value), GetValue(key, parentNodeName, winEvent), aliasFactory: name => Aliases.Instance.Value.Items.TryGetValue(name, out var found) ? found : null, fieldName => GetValue(fieldName, parentNodeName, winEvent), winEvent.EnumerateStringValues, domainControllers, canProcessRegex, onRegexFailure);
    }
