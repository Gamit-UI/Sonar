...
internal static class AuditPolicyMapping
{
    static AuditPolicyMapping()
    {
        foreach (var kvp in EventIdBySubcategoryGuid)
        {
            if (VolumeBySubcategoryGuid.TryGetValue(kvp.Key, out var volume))
            {
                foreach (var eventId in kvp.Value.Select(v => v.EventId))
                {
                    if (VolumeByEventId.TryGetValue(eventId, out var currentVolume))
                    {
                        if (currentVolume < volume)
                        {
                            VolumeByEventId[eventId] = volume;
                        }
                    }
                    else
                    {
                        VolumeByEventId.Add(eventId, volume);
                    }
                }
            }
        }
    ...
    public static readonly IDictionary<int, AuditPolicyVolume> VolumeByEventId = new Dictionary<int, AuditPolicyVolume>();
    ...
    private static readonly IDictionary<Guid, ISet<AuditPolicyEventId>> EventIdBySubcategoryGuid = new Dictionary<Guid, ISet<AuditPolicyEventId>>
    {
        {
