...
internal static class ProfileHelper
{
    public static bool ShouldBeEnabled(string ruleId, DetectionProfile profile, DetectionSeverity severity, DetectionStatus status, AuditPolicyVolume volume)
    {
        ...
        if (profile.HasFlag(DetectionProfile.CorePlusPlus))
        {
            return volume < AuditPolicyVolume.VeryHigh && status is DetectionStatus.Stable or DetectionStatus.Test or DetectionStatus.Experimental && severity >= DetectionSeverity.Medium;
        }

        if (profile.HasFlag(DetectionProfile.CorePlus))
        {
            return volume < AuditPolicyVolume.VeryHigh && status is DetectionStatus.Stable or DetectionStatus.Test && severity >= DetectionSeverity.Medium;
        }

        if (profile.HasFlag(DetectionProfile.Core))
        {
            return volume < AuditPolicyVolume.VeryHigh && ShouldBeEnabledForCore(ruleId, severity, status);
        }
